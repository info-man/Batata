<%@ Page Title="CMSCK - Gestor de Conteúdos" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="PesquisaBanner.aspx.cs" Inherits="MASTERSEGUROS.WEB.PesquisaBanner" %>

<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <asp:ScriptManager ID="ScriptManager1" runat="server"></asp:ScriptManager>
    <h3>Banners > Pesquisa de Banners</h3>
    <br />
   <asp:UpdatePanel ID="UpdatePanel2" runat="server"> 
        ...
        <table border="0" cellpadding="3" cellspacing="0" style="vertical-align: top">
                            <tr>
                                <td>ID Banner:</td>
                                <td>
                                    <asp:TextBox ID="BannerIDTextBox" runat="server"  Width="30" MaxLength="4" />&nbsp;&nbsp;ID Classificador:&nbsp;<asp:TextBox ID="ClassificadorIDTextBox" runat="server" Width="30" MaxLength="4" />
                            ...
                            <tr>
                                <td>URL:</td>
                                <td>
                                    <asp:TextBox ID="URLTextBox" runat="server"  Width="398" MaxLength="400" />
                                </td>
                            </tr>
                            <tr>
                                <td>Data:</td>
        ...
        <div id="dialog-confirmDeleteBanner" title="CMSCK" style="display: none">
        <br />
        <b>Tem a certeza que pretende remover o Banner?</b>
    </div>

        <asp:GridView ID="GridViewBanners" runat="server"
        AllowPaging="True" AutoGenerateColumns="False"
        CellPadding="2" DataKeyNames="BannerID,Lingua" DataSourceID="SqlDataSourceBanner"
        ...
        <Columns>
            <asp:BoundField DataField="BannerID" HeaderText="ID" SortExpression="BannerID" ItemStyle-Width="50px" />
            ...
            <asp:TemplateField HeaderText="Estado" SortExpression="Estado" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="50px">
                <ItemTemplate>  
                    <asp:ImageButton runat="server" ID="imgEstado" OnClick="imgEstado_Click" Estado='<%#(object)Eval("Estado")%>' IDBanner='<%#(object)Eval("BannerID")%>' IDLingua='<%#(object)Eval("Lingua")%>'  ImageUrl='<%# BuildEstado((object)Eval("Estado"))%>' ToolTip='<%# BuildToolTipEstado((object)Eval("Estado"),(object)Eval("Titulo"))%>' />
                </ItemTemplate>
             </asp:TemplateField>
             ...
             <asp:TemplateField HeaderText="Operações" ItemStyle-HorizontalAlign="Center">
                <ItemTemplate>
                    <asp:DropDownList runat="server" ID="ddlOperation" AutoPostBack="true" IDBanner='<%#(object)Eval("BannerID")%>' IDLingua='<%#(object)Eval("Lingua")%>' AppendDataBoundItems="true" Font-Size="12px"  onchange="javascript:if (this.value=='0') {return false;} if (this.value=='2') {return ConfirmDeleteBanner(this);} else {true; }" OnSelectedIndexChanged="Operation_SelectedIndexChanged">
   ...
   <asp:SqlDataSource ID="SqlDataSourceBanner" OnUpdated="On_Updated"  OnInserted="On_Inserted" runat="server"
        ConnectionString="<%$ ConnectionStrings:DBConnectionString %>"
        SelectCommandType="StoredProcedure" SelectCommand="spGetListBanner">
       <SelectParameters>
           <asp:ControlParameter ControlID="BannerIDTextbox" PropertyName="Text" Name="BannerID" Type="Int32" DefaultValue="-1" />
           ...
           <asp:ControlParameter ControlID="URLTextbox" PropertyName="Text" Name="URL" DefaultValue="NULL"  Type="string" />
   ...
   <asp:SqlDataSource ID="SqlDataSourceTipo" runat="server"
        ConnectionString="<%$ ConnectionStrings:DBConnectionString %>"
        SelectCommand="SELECT * FROM [cmsck_tipo_Media] "></asp:SqlDataSource>
   ...
   <script type="text/javascript">
    ...
    function ConfirmDeleteBanner(obj) {
        $("#dialog-confirmDeleteBanner").dialog({
