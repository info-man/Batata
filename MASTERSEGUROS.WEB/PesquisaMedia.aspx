<%@ Page Title="CMSCK - Gestor de Conteúdos" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="PesquisaMedia.aspx.cs" Inherits="MASTERSEGUROS.WEB.PesquisaMedia" %>

<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <asp:ScriptManager ID="ScriptManager1" runat="server"></asp:ScriptManager>
    <h3>Medias > Pesquisa de Medias</h3>
    <br />
   <asp:UpdatePanel ID="UpdatePanel2" runat="server"> 
        ...
        <table border="0" cellpadding="3" cellspacing="0" style="vertical-align: top">
                            <tr>
                                <td>ID Media:</td>
                                <td>
                                    <asp:TextBox ID="MediaIDTextBox" runat="server"  Width="30" MaxLength="4" />&nbsp;&nbsp;ID Classificador:&nbsp;<asp:TextBox ID="ClassificadorIDTextBox" runat="server" Width="30" MaxLength="4" />
        ...
        <div id="dialog-confirmDeleteMedia" title="CMSCK" style="display: none">
        <br />
        <b>Tem a certeza que pretende remover o Media?</b>
    </div>

        <asp:GridView ID="GridViewMedias" runat="server"
        AllowPaging="True" AutoGenerateColumns="False"
        CellPadding="2" DataKeyNames="MediaID,Lingua" DataSourceID="SqlDataSourceMedia"
        ...
        <Columns>
            <asp:BoundField DataField="MediaID" HeaderText="ID" SortExpression="MediaID" ItemStyle-Width="50px" />
            ...
            <asp:TemplateField HeaderText="Estado" SortExpression="Estado" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="50px">
                <ItemTemplate>  
                    <asp:ImageButton runat="server" ID="imgEstado" OnClick="imgEstado_Click" Estado='<%#(object)Eval("Estado")%>' IDMedia='<%#(object)Eval("MediaID")%>' IDLingua='<%#(object)Eval("Lingua")%>'  ImageUrl='<%# BuildEstado((object)Eval("Estado"))%>' ToolTip='<%# BuildToolTipEstado((object)Eval("Estado"),(object)Eval("Titulo"))%>' />
                </ItemTemplate>
             </asp:TemplateField>
             ...
             <asp:TemplateField HeaderText="Operações" ItemStyle-HorizontalAlign="Center">
                <ItemTemplate>
                    <asp:DropDownList runat="server" ID="ddlOperation" AutoPostBack="true" IDMedia='<%#(object)Eval("MediaID")%>' IDLingua='<%#(object)Eval("Lingua")%>' AppendDataBoundItems="true" Font-Size="12px"  onchange="javascript:if (this.value=='0') {return false;} if (this.value=='2') {return ConfirmDeleteMedia(this);} else {true; }" OnSelectedIndexChanged="Operation_SelectedIndexChanged">
   ...
   <asp:SqlDataSource ID="SqlDataSourceMedia" OnUpdated="On_Updated"  OnInserted="On_Inserted" runat="server"
        ConnectionString="<%$ ConnectionStrings:DBConnectionString %>"
        SelectCommandType="StoredProcedure" SelectCommand="spGetListMedia">
       <SelectParameters>
           <asp:ControlParameter ControlID="MediaIDTextbox" PropertyName="Text" Name="MediaID" Type="Int32" DefaultValue="-1" />
   ...
   <script type="text/javascript">
    ...
    function ConfirmDeleteMedia(obj) {
        $("#dialog-confirmDeleteMedia").dialog({
