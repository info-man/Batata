<%@ Page Title="Consultar Apólices" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="PesquisaClientePopup.aspx.cs" Inherits="MASTERSEGUROS.WEB.PesquisaClientePopup" %>
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="Província" SortExpression="ProvinciaID" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="70px">
             ...
             <asp:TemplateField HeaderText="Documentos" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="70px">
                <ItemTemplate>
                    <asp:DropDownList runat="server" ID="ddlOperation" ESTADOAPLID='<%#(object)Eval("EstadoApoliceID")%>'   APLID='<%#(object)Eval("ApoliceID")%>'  style="font-family:Open Sans;font-size: 11px;padding: 0px;margin: 3px;"  OnChange="javascript:if (this.value==0) return false; else window.open('AutoDocument.aspx?docID=' + this.value + '&aplID=' + this.getAttribute('APLID'), 'blank');this.value=0">
